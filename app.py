...
from auth import (AuthError,
                  requires_auth,
                  get_auth0_user_id_from_cookie_token,
                  check_auth)
...
def get_current_member_object():
    current_user = Member.query.filter_by(
        auth0_user_id=get_auth0_user_id_from_cookie_token()).one_or_none()
    return current_user
#
...
def create_app(test_config=None, database_path=db_path):
    ...
    def index():
        ...
        past_events = Event.query.filter(
            Event.time <= datetime.now()).order_by(
            Event.time.desc()).limit(5)
    ...
    def get_all_members():
        ...
        members = Member.query.filter_by(
            member=True).order_by(
            Member.username).all()
        members_num = len(members)
        guests = Member.query.filter_by(
            member=False).order_by(
            Member.username).all()
    ...
    def get_event_page(event_id):
        ...
        return render_template(
            'pages/event.html',
            event=event,
            current_players_num=current_players_num,
            did_join=did_join,
            is_host=is_host,
            current_user=current_user)
    ...
    def join_event(event_id):
        try:
            event = Event.query.filter_by(id=event_id).one_or_none()
            current_user = get_current_member_object()
            if current_user not in event.players and len(
                    event.players) < event.max_players:
                event.players.append(current_user)
            else:
                if len(event.players) >= event.max_players:
                    # not sure which code should I respond with here
                    return jsonify(
                        {'success': False,
                         'message': 'This event has already ' +
                         'reached maximum capacity'}), 400
            db.session.commit()
            db.session.close()
    ...
    def edit_event(event_id):
        ...
        try:
            ...
            games = [Game.query.filter_by(id=game_id).one_or_none(
        ...
        try:
            ...
            return render_template(
                ...
                member=Member.query.filter_by(
                    auth0_user_id=auth0_user_id).one_or_none(),
                current_user=Member.query.filter_by(
                    auth0_user_id=auth0_user_id).one_or_none())
    ...
    def search_results_member():
        ...
        results = Member.query.filter(
            Member.username.ilike(f'%{search}%')).all()
        count = len(results)
        return render_template(
            'pages/search_member.html',
            search=search,
            results=results,
            count=count,
            current_user=current_user)
    ...
    def search_results_event():
        ...
        results = Event.query.filter(
            Event.name.ilike(f'%{search}%')).order_by(
            Event.time).all()
        count = len(results)
        return render_template(
            'pages/search_event.html',
            search=search,
            results=results,
            count=count,
            current_user=current_user)
    ...
    def delete_member(member_id):
        try:
            ...
            if member is None:
                abort(404)
            # This will delete games if they are no longer owned by any user
            for game in member.ownership:
